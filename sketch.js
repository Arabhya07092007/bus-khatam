...
let plr1k=0, plr2k=0, plr3k=0, plr4k=0;
...
function draw(){
    ...
    if(gameState === 1){
      game.play();
      for(var tlr in khiladies){
        if(khiladies[tlr].name!==undefined){
     
        push();
        textSize(40);
        fill("indigo");
        text(khiladies[tlr].name+":"+khiladies[tlr].kills,khiladies[tlr].body.x,khiladies[tlr].body.y);

        rectMode(CENTER);
        fill('pink')
        rect(khiladies[tlr].body.x-20,khiladies[tlr].body.y+20,khiladies[tlr].health,20);
        pop();}
      }
    }
...
w1 = createSprite(1000,1000,30000,50);
w1.shapeColor = "red";
spritesGroup.push(w1);

    invisibleObject  = createSprite(1000,0,1000,150);
